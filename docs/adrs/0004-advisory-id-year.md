---
id: 0004
title: Year in OpenACA-YYYY-NNNN is assignment year, not alias year
status: superseded
date: 2026-05-08
supersedes: null
superseded-by: 0009
...
OpenACA advisories carry IDs of the form `OpenACA-YYYY-NNNN` and frequently
alias upstream CVE/GHSA records whose own year component reflects a
different timeline. The first batch of V0 advisories (`CVE-2026-0001`
through `CVE-2026-0005`) all alias CVE-2025-* IDs, which prompted a
reasonable question: should OpenACA match the upstream year to avoid
visual confusion?

## Decision

The `YYYY` component of an OpenACA ID is the year **OpenACA assigned the
ID**, not the year of any aliased upstream record. CVE-2026-0001 means
"OpenACA catalogued this in 2026" — the aliased CVE-2025-XXXXX year is
...
- **Drop the year entirely (`OpenACA-NNNNNN`)**: cleaner, no apparent
...
- Visual mismatch between `OpenACA-YYYY` and aliased `CVE-YYYY`/`GHSA-YYYY`
...
- Backfilling old vulnerabilities into OpenACA in 2027 will produce
  `OpenACA-2027-NNNN` IDs, which is correct under this convention even
  though the underlying flaw may date to 2024.

## When to revisit

If a downstream consumer requires alias-year alignment as a hard
constraint (no current evidence this exists), or if OpenACA ever issues
...
"when OpenACA recorded this." Neither is on the roadmap.
