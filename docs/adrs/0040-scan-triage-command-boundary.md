...
workflow concern, the CLI boundary becomes blurry and future Cloud/plugin
...
does not name the decision layer that Cloud, the Claude Code plugin, and
concierge assessments all need to share.
...
Both paths use the same triage engine. The Claude Code plugin and future Cloud
ingest should call that shared engine instead of inventing report logic.
...
- **Build report generation only in Cloud**: rejected because local OSS and the
  Claude Code plugin need a forwardable single-endpoint report before a team
  adopts Cloud. Cloud should aggregate the same triage model across endpoints,
  not define a separate model.
...
triage, if Cloud requires materially different ranking semantics from local
reports, or if the CLI surface becomes confusing enough that maintaining both
`scan --report exposure` and `triage` harms usability.
