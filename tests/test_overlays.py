...
def test_apply_overlays_merges_agent_context_by_alias_intersection():
    ...
    openaca = merged[0]["database_specific"]["openaca"]
    assert openaca["source"] == "osv.dev"
    assert openaca["overlay_source"] == "openaca.dev"
    assert openaca["taxonomies"] == {"owasp_agentic_top10": ["asi05"]}
    assert openaca["evidence_level"] == "confirmed"


def test_apply_overlays_ignores_unrelated_records():
    record = {"id": "GHSA-1111", "database_specific": {"openaca": {"source": "osv.dev"}}}
    overlay = {
        "id": "GHSA-2222",
        "database_specific": {
            "openaca": {
