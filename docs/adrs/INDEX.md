...
- [ADR-0025 — Treat Fleet upload as endpoint inventory with a narrow hygiene contract](0025-fleet-upload-contract.md): supersedes ADR-0024's redaction requirement; Fleet upload remains explicit and opt-in, but the payload contract is endpoint inventory plus a small exclusion list for source code, raw config bodies, env values, detected secrets, and full shell argv. Manual BOM upload is deferred from V0.
...
- [ADR-0024 — Keep Fleet collection as an opt-in CLI surface](0024-opt-in-fleet-cli.md): superseded by ADR-0025 for the upload contract. The opt-in `openaca fleet ...` command boundary remains, but comprehensive redaction before upload/cache is replaced by an endpoint-inventory payload contract with a narrow high-risk-data exclusion list.
