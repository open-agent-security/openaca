...
|---|---|---|---|
| `attributed_to` | string \| absent | `"plugin/<name>"`, `"plugin/<name>@<version>"`, `"plugin/<marketplace>/<name>"`, or `"plugin/<marketplace>/<name>@<version>"` | Derived from the composition graph — the nearest plugin ancestor of the finding's component (ADR-0037). Repo-mode plugins (`.claude-plugin/plugin.json`) produce a two-segment identity (`plugin/<name>`); marketplace plugins produce a `plugin/<marketplace>/<name>` identity. `@<version>` is appended only when the plugin carries a version field. Note: `<name>` may be a scoped npm package (e.g. `@acme/tool`), producing a value like `plugin/market/@acme/tool` that contains more than two forward slashes after `plugin/` — parsers must not assume exactly two slash-delimited segments after the `plugin/` prefix. Absent for direct components with no plugin ancestor (declared in settings, repo manifests, or host repo lockfiles). |
...
```json
{
  ...
  "locations": [/* ... */],
  "properties": {
    "attributed_to": "plugin/claude-plugins-official/superpowers@5.1.0",
    "coverage": "transitive",
    "transitive": true,
    "source": "osv.dev",
