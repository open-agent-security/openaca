...
- [ADR-0025 — OSS install via uv + curl-pipe-sh; defer single-binary distribution](0025-curl-pipe-sh-install-via-uv.md): Ship `openaca.dev/install.sh` (persistent install via `uv tool install`) and `openaca.dev/scan` (ephemeral run via `uvx`) as the leading OSS install paths; `pip install` and manual `uv tool install` demoted to "manual install" docs. Defers pyapp single-binary and Rust/Go rewrite until friction is measurably the remaining bottleneck; `OPENACA_VERSION` env var pins for Fleet/MDM/CI reproducibility while OSS defaults to `latest`.
