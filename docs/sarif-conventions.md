...
|---|---|---|---|
| `attributed_to` | string \| absent | `"plugin/<marketplace>/<name>@<version>"` | Derived from the composition graph — the nearest plugin ancestor of the finding's component (ADR-0037). Absent for direct components with no plugin ancestor (declared in settings, repo manifests, or host repo lockfiles). |
