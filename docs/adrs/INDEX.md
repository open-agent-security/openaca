...
- [ADR-0006 — asve-scan subcommands, claude-plugin ecosystem, attribution](0006-asve-scan-subcommands-and-attribution.md): Trivy-style `repo`/`fs` subcommand split (subcommand is **required**; no no-subcommand fallback — the back-compat shim was removed pre-launch); `claude-plugin` as a recognized `affected[*].package.ecosystem` so plugin advisories match via `_match_versioned`; `attributed_to` mirrored on ComponentRef and Finding so plans 008 and 009 can tag "via plugin X" findings.
