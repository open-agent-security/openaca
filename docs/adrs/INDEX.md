...
- [ADR-0009 — V0 ships ASVE overlays, not ASVE advisory IDs](0009-overlay-only-v0.md): V0 overlays use upstream IDs (`GHSA-*`, `CVE-*`) and carry only `database_specific.asve`; scanner queries OSV.dev by PURL, dedupes by alias graph, then applies bundled overlays; no `--db` or `--overlays` CLI flag in V0.
- [ADR-0010 — Use deterministic candidate seeding and nested overlay taxonomies](0010-overlay-taxonomies-and-seeding.md): V0 seeders write deterministic OSV-derived candidates outside `overlays/`; humans promote via projection; ASVE taxonomy mappings live under `database_specific.asve.taxonomies`; malicious package context uses `threat_kind`, not a top-level record type.

## Superseded

- [ADR-0003 — Single namespace, type-tagged advisories](0003-single-namespace-architecture.md): superseded by ADR-0009; V0 no longer mints ASVE advisory IDs.
