...
optional — the three layers downstream consumers roll up on.
...
- **Source-stable, optional component identity (ADR-0042, plan 038).**
  ...
  join key does not. Every component keeps a BOM-unique `bom-ref`
...
- **`openaca:identity` is now optional.** Alias-derived identities
  ...
  nullable cross-BOM join key and use `bom-ref` as the per-BOM occurrence
