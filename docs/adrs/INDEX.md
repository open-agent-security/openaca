...
- [ADR-0024 — Keep Fleet collection as an opt-in CLI surface](0024-opt-in-fleet-cli.md): Fleet upload lives behind explicit `openaca fleet ...` commands in the public CLI; local scan/BOM flows remain upload-free, backend URL is configurable, redaction runs before upload/cache, and launchd/MDM recipes schedule the CLI instead of shipping a daemon.
