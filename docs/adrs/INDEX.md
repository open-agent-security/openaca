...
- [ADR-0017 — Endpoint scan defaults project context to cwd](0017-endpoint-scan-cwd-project-default.md): amends ADR-0006's "project settings are opt-in" framing — `openaca scan endpoint` now defaults `--project` to the current working directory (matching the Claude Code mental model), with `--no-project` as the explicit opt-out and an always-emitted `detected config_dir=..., project=...` line for transparency. Driven by round-1 beta evidence that the opt-in default produced 0-component scans on real Claude Code project dirs.
