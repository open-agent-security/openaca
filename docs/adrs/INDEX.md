...
- [ADR-0018 — Generic skill ecosystem and Agent BOM-compatible metadata](0018-generic-skill-ecosystem-and-agent-bom-fields.md): supersedes ADR-0007 for skill ecosystem naming; `skill` is the canonical cross-runtime ecosystem/component identity prefix, `claude-skill` is a pre-release matcher alias only, and scanner metadata separates component identity, source identity, evidence, activation context, and relationships for future Agent BOM export.
