...
def test_unversioned_plugin_without_commit_sha_is_flagged():
    ref = ComponentRef(
        name="feature-dev",
        version="unknown",
        component_identity="plugin/official/feature-dev",
    ...
    assert len(findings) == 1
    assert findings[0].rule_id == "openaca-posture-mutable-install-reference"
    assert findings[0].component_label == "plugin/official/feature-dev@unknown"
...
def test_unversioned_plugin_with_commit_sha_is_not_flagged():
    ref = ComponentRef(
        ...
        version="unknown",
        component_identity="plugin/reboot-skills/reboot-chat-app",
        source_manifest="installed_plugins.json",
