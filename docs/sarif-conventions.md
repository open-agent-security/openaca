...
|---|---|---|---|
| `attributed_to` | string \| absent | `"plugin/<name>"`, `"plugin/<name>@<version>"`, `"plugin/<marketplace>/<name>"`, or `"plugin/<marketplace>/<name>@<version>"` | Derived from the composition graph — the nearest plugin ancestor of the finding's component (ADR-0037). Repo-mode plugins (`.claude-plugin/plugin.json`) produce a two-segment identity (`plugin/<name>`); marketplace plugins produce a three-segment identity (`plugin/<marketplace>/<name>`). `@<version>` is appended only when the plugin carries a version field. Absent for direct components with no plugin ancestor (declared in settings, repo manifests, or host repo lockfiles). |
